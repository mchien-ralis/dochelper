      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        ...
        <div style={{ display: 'flex', gap: '10px' }}>
          <button onClick={fetchRequests}
            style={{ padding: '10px 16px', background: '#f0f0f0', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '14px' }}>
            🔄 Refresh
          </button>
        ...
        <div>
          {loading ? (
            ...
            <div style={{ textAlign: 'center', padding: '40px', color: '#6b7280', background: '#f9fafb', borderRadius: '8px' }}>
              <div style={{ fontSize: '32px', marginBottom: '12px' }}>📭</div>
              <p>No requests yet.</p>
              <a href="/create" style={{ color: '#2563eb' }}>Create your first request →</a>
            </div>
          ) : (
            requests.map((req) => {
